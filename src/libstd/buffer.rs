...
pub fn from_string_utf8(state: StateContainer, args: Vec<Container>, _: Gi) -> Result<Container, Container> {
    if args.len() == 0 {
        return Err(make_err("buffer.from_string_utf8 requires 1 argument"));
    }
    let s = to_string_base(state.clone(), args[0].clone())?;
    let s = s.as_bytes().to_vec();
    new_from_vec(state.clone(), s)
}

pub fn from_string_utf16be(state: StateContainer, args: Vec<Container>, _: Gi) -> Result<Container, Container> {
    if args.len() == 0 {
...
pub fn from_string_utf16le(state: StateContainer, args: Vec<Container>, _: Gi) -> Result<Container, Container> {
    if args.len() == 0 {
...
pub fn new_from_vec(state: StateContainer, data: Vec<u8>) -> Result<Container, Container> {
...
fn meta_to_string(_: StateContainer, args: Vec<Container>, _: Gi) -> Result<Container, Container> {
    if args.len() == 0 {

...
pub fn cycle(state: StateContainer, args: Vec<Container>, _: Gi) -> Result<Container, Container> {
    if args.len() == 0 {
    ...
    let g = {
        ...
        obj.internals.insert(0, array::new_with_vec(state.clone(), vec![])?);
        ...
        Function {
            ...
            call: |state, _, gi| {
                ...
                if b {
                    ...
                    call_prop(state.clone(), arr.clone(), vec![rv.clone()], "push".to_string())?;
                    Ok(make_tuple(vec![make_container(Value::Boolean(true)), rv]))
                } else {
                    let f = call_prop(state.clone(), arr, Vec::new(), "iter".to_string())?;
                    obj.internals.insert(1, f);
                    obj.internals.insert(0, array::new_with_vec(state.clone(), vec![])?);
                    let arr = obj.internals[&0].clone();
                    let f = obj.internals[&1].clone();
